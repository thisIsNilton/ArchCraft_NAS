...
Inc_cls = 10  # Número de classes incrementadas a cada passo

def run_test():
    indi_no = 0
    code = [2, 40, [1, 1, 3, 3, 3], [0, 0, 2, 2, 2]]  # AlexAC-A, about 6.28M
...
class TrainModel(object):
    ...
    def process(self, s):
        ...
        df0.columns = [f'task_{i+1}' for i in range(len(taskcla))]
        df0.index = [f'task_{i+1}' for i in range(len(taskcla))] # Opcional: definir nomes para as linhas também

        df0.to_parquet("./accuracy_ds_double.parquet")
        print("Matriz de acurácia salva com nomes de colunas em accuracy.parquet")

        df0 = pd.DataFrame(self.dict_forgetting)
        df0.to_parquet("./forgetting_ds_double.parquet")
        print("Matriz de acurácia salva com nomes de colunas em forgetting.parquet")
